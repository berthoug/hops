...
public class CSQueueInfo {
  ...
  private static final Log LOG = LogFactory.getLog(SchedulerApplicationInfo.class);
  ...
  public void persist(CSQueueDataAccess csQDA, CSLeafQueueUserInfoDataAccess csLeafQUI) throws StorageException {
    persistCSQueueInfoToAdd(csQDA);
    persistCSLeafQueueInfoToAdd(csLeafQUI);
    persistCSLeafQueueUsersToRemove();
  }

  private void persistCSQueueInfoToAdd(CSQueueDataAccess QMDA) throws StorageException {
    if (csQueueToAdd != null) {
      List<io.hops.metadata.yarn.entity.capacity.CSQueue> toAddQueues = new ArrayList<io.hops.metadata.yarn.entity.capacity.CSQueue>();
      for (String csQueueName : csQueueToAdd.keySet()) {
        ...
        io.hops.metadata.yarn.entity.capacity.CSQueue hopCSQueue = new io.hops.metadata.yarn.entity.capacity.CSQueue(
        ...
        LOG.info("persistCSQueueInfoToAdd :  QueueName" + csQueueAdd.getQueueName()
  ...
  private void persistCSLeafQueueInfoToAdd(CSLeafQueueUserInfoDataAccess QMDA) throws StorageException {
    if (csLeafQueueUserInfoToAdd != null) {
      List<io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo> toAddQueues = new ArrayList<io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo>();
      for (String username : csLeafQueueUserInfoToAdd.keySet()) {

        CSLeafQueueUserInfo csLeafQueueUser = csLeafQueueUserInfoToAdd.get(username);
        io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo hopCSLeafQueueUserInfo = new io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo(
                username,
  ...
  public void persistCSLeafQueueUsersToRemove() throws StorageException {
    if (usersToRemove != null && !usersToRemove.isEmpty()) {
      CSLeafQueueUserInfoDataAccess csLQueueDA = (CSLeafQueueUserInfoDataAccess) RMStorageFactory.getDataAccess(CSLeafQueueUserInfoDataAccess.class);
      List<io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo> usersToRemoveList = new ArrayList<io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo>();
      for (String userName : usersToRemove.keySet()) {
        usersToRemoveList.add(new io.hops.metadata.yarn.entity.capacity.CSLeafQueueUserInfo(userName, 0, 0, 0, 0));
