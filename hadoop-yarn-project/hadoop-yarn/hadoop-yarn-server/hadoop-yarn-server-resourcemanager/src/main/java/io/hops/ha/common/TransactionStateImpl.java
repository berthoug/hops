...
public class TransactionStateImpl extends TransactionState {
  ...
  public void persistFairSchedulerNodeInfo(FSSchedulerNodeDataAccess FSSNodeDA, ResourceDataAccess resourceDA,
    ...
    fairschedulerNodeInfo.persist(FSSNodeDA, resourceDA, rmcontainerDA, launchedContainersDA, appSDA);
  }
  ...
  private void persistRMContainerToUpdate() throws StorageException {
    if (!rmContainersToUpdate.isEmpty()) {
      ...
      for (String containerId : rmContainersToUpdate.keySet()) {
        ...
        String sReserveStatus = rmContainer.getState().toString();
  ...
  private void persistFiCaSchedulerNodeToRemove(ResourceDataAccess resourceDA, FiCaSchedulerNodeDataAccess ficaNodeDA, RMContainerDataAccess rmcontainerDA, LaunchedContainersDataAccess launchedContainersDA) throws StorageException {
    if (!ficaSchedulerNodeInfoToRemove.isEmpty()) {
      ...
      for (String nodeId : ficaSchedulerNodeInfoToRemove.keySet()) {
        ...
        if (container != null) {
	  //TOVERIFY can we get a nulpointer exception
          rmcontainerToRemove.add(
	      new RMContainer(container.getContainerId().toString(),
                  container.getApplicationAttemptId().toString(),
                  container.getNodeId().toString(),
                  container.getUser(),
                  container.getReservedNode().toString(),
                  Integer.MIN_VALUE,
                  container.getReservedResource().getMemory(),
                  container.getReservedResource().getVirtualCores(),
                  container.getStartTime(),
                  container.getFinishTime(),
                  container.getState().toString(),
                  container.getReservedNode().getHost(),
                  container.getReservedNode().getPort(),
  ...
  public void persistFiCaSchedulerNodeToAdd(ResourceDataAccess resourceDA,
    ...
    if (!ficaSchedulerNodeInfoToAdd.isEmpty()) {
	  ...
	  //TOVERIFY : what if we persist for the capacity scheduler?!
          toAddFiCaSchedulerNodes.add(
              new FiCaSchedulerNode(nodeId, node.getNodeName(),
				    node.getNumContainers(),null));
          //Add Resources
		...
		node.getReservedContainer().getContainerId().toString(),
                node.getReservedContainer().getApplicationAttemptId()
                  ...
                  node.getReservedContainer().getReservedNode().getHost(),
                  node.getReservedContainer().getReservedNode().getPort(),
