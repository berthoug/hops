...
public class CapacityScheduler extends AbstractYarnScheduler
    ...
    private Map<String, CSQueue> queues = new ConcurrentHashMap<String, CSQueue>();  //recovered

    private Map<NodeId, FiCaSchedulerNode> nodes
            = new ConcurrentHashMap<NodeId, FiCaSchedulerNode>();   //recovered

    private Resource clusterResource
            = RecordFactoryProvider.getRecordFactory(null).newRecordInstance(Resource.class);  // recovered
     ...
     * @param cs
     */
    static void schedule(CapacityScheduler cs, TransactionState transactionState) {
    ...
    /**
     * Add the new queues (only) to our list of queues... ... be careful, do not
     * overwrite existing queues.
     *
     * @param queues
     * @param newQueues
     */
    ...
    private synchronized void addApplication(ApplicationId applicationId,
        ...
        try {
            ...
            this.rmContext.getDispatcher().getEventHandler()
                    .handle(new RMAppRejectedEvent(applicationId, ace.toString(), transactionState));
            return;
        }
        org.apache.hadoop.yarn.server.resourcemanager.scheduler.SchedulerApplication application
                = new org.apache.hadoop.yarn.server.resourcemanager.scheduler.SchedulerApplication(queue, user);
    ...
    private synchronized void addApplicationAttempt(
        ...
        if(transactionState != null) {
          ((TransactionStateImpl) transactionState).getSchedulerApplicationInfo().setFiCaSchedulerAppInfo(attempt);
        }
        
        queue.submitApplicationAttempt(attempt, application.getUser(), transactionState);
    ...
    private synchronized void doneApplication(ApplicationId applicationId,
        ...
        org.apache.hadoop.yarn.server.resourcemanager.scheduler.SchedulerApplication application = applications.get(applicationId);
        if (application == null) {
        ...
        if (transactionState != null) {
            ((TransactionStateImpl) transactionState).getSchedulerApplicationInfo().setApplicationIdtoRemove(applicationId);
        }
    }
    ...
    private synchronized void doneApplicationAttempt(
        ...
        if (!(queue instanceof LeafQueue)) {
            ...
            queue.finishApplicationAttempt(attempt, queue.getQueueName(), transactionState);
        }
    }
    ...
    public Allocation allocate(ApplicationAttemptId applicationAttemptId,
            ...
            List<String> blacklistAdditions, List<String> blacklistRemovals, TransactionState transactionState) {
        ...
        synchronized (application) {
            ...
            return application.getAllocation(getResourceCalculator(),
                    clusterResource, getMinimumResourceCapability(), transactionState);
        }
    }
    ...
    private synchronized void nodeUpdate(RMNode nm, TransactionState transactionState) {
        if (LOG.isDebugEnabled()) {
        ...
        SchedulerUtils.updateResourceIfChanged(node, nm, clusterResource, LOG, transactionState);
            if (transactionState != null) {
        ...
        for (ContainerStatus launchedContainer : newlyLaunchedContainers) {
            containerLaunchedOnNode(launchedContainer.getContainerId(), node, transactionState);
        }
    ...
    private synchronized void allocateContainersToNode(FiCaSchedulerNode node, TransactionState transactionState) {
        ...
        if (reservedContainer != null) {
            ...
            CSAssignment assignment = queue.assignContainers(clusterResource, node, transactionState);
    ...
    private void containerLaunchedOnNode(ContainerId containerId, FiCaSchedulerNode node, TransactionState transactionState) {
        ...
        if (application == null) {
            ...
            this.rmContext.getDispatcher().getEventHandler()
                    .handle(new RMNodeCleanContainerEvent(node.getNodeID(), containerId, transactionState));
            return;
        }

        application.containerLaunchedOnNode(containerId, node.getNodeID(), transactionState);
    }
    ...
    public void handle(SchedulerEvent event) {
        ...
        switch (event.getType()) {
            ...
            case NODE_UPDATE: {
                ...
                if (!scheduleAsynchronously) {
                    allocateContainersToNode(getNode(node.getNodeID()), event.getTransactionState());
                }
            }
            break;
            case APP_ADDED: {
    ...
    private synchronized void addNode(RMNode nodeManager, TransactionState transactionState) {
        ...
        if(transactionState != null) {
          ((TransactionStateImpl) transactionState).addFicaSchedulerNodeInfoToAdd(nodeManager.getNodeID().toString(), ficaNode);
          ((TransactionStateImpl) transactionState).updateClusterResource(clusterResource);
        }
    ...
    private synchronized void removeNode(RMNode nodeInfo, TransactionState transactionState) {
        ...
        if (transactionState != null) {
            ((TransactionStateImpl) transactionState).addFicaSchedulerNodeInfoToRemove(nodeInfo.getNodeID().toString(), node);
        }
    ...
    private synchronized void completedContainer(RMContainer rmContainer,
        ...
        queue.completedContainer(clusterResource, application, node,
                rmContainer, containerStatus, event, null, transactionState);
    ...
    public void recover(RMState state) throws Exception {
        try {
            ...
            Map<String, CSLeafQueueUserInfo> queueNameAndHopCSLeafQueueUser = new HashMap<String, CSLeafQueueUserInfo>();
            ...
            for(QueueMetrics qMetrics : recoverQueueMetrics){
                ...
                metrics = org.apache.hadoop.yarn.server.resourcemanager.scheduler.QueueMetrics.forQueue(qMetrics.getQueuename(), null, false, conf);
            ...
            for (SchedulerApplication fsapp : appsMap.values()) {
                ...
                AppSchedulingInfo hopFiCaSchedulerApp = state.getAppSchedulingInfo(fsapp.getAppid());

                if (hopFiCaSchedulerApp != null) {
                    //construct SchedulerAppliaction
                    org.apache.hadoop.yarn.server.resourcemanager.scheduler.SchedulerApplication app = new org.apache.hadoop.yarn.server.resourcemanager.scheduler.SchedulerApplication(getQueue(hopFiCaSchedulerApp.getQueuename()), fsapp.getUser());
                    //construct ApplicationAttemptId
                    ApplicationAttemptId appAttemptId = ConverterUtils.toApplicationAttemptId(hopFiCaSchedulerApp.getSchedulerAppId());
            ...
            io.hops.metadata.yarn.entity.Resource recovered = state.getResource("cluster", io.hops.metadata.yarn.entity.Resource.CLUSTER, io.hops.metadata.yarn.entity.Resource.AVAILABLE);
            if(recovered != null) {
...
//            }
            ...
            for (CSLeafQueueUserInfo hopCSLeafQueue : recoverCSLeafQueueUser) {
                queueNameAndHopCSLeafQueueUser.put(hopCSLeafQueue.getUserName(), hopCSLeafQueue);
            }
            
            for (io.hops.metadata.yarn.entity.capacity.CSQueue hopQueue : recoverCSQueues) {

                CSQueue csQueue = queues.get(hopQueue.getName());

                csQueue.setAbsoluteUsedCapacity(hopQueue.getAbsoluteUsedCapacity(), null);
                csQueue.setUsedCapacity(hopQueue.getUsedCapacity(), null);

                csQueue.getUsedResources().setMemory(hopQueue.getUsedResourceMemory());
                csQueue.getUsedResources().setVirtualCores(hopQueue.getUsedResourceVCores());
                ...
                if (!hopQueue.isParent()) {
                    ...
                    if (queueNameAndHopCSLeafQueueUser.containsKey(username)) {
                        Resource consumedresource = BuilderUtils.newResource(
                                queueNameAndHopCSLeafQueueUser.get(username).getConsumedMemory(),
                                queueNameAndHopCSLeafQueueUser.get(username).getConsumedVCores());
                        
                        LeafQueue leafQueue = (LeafQueue) csQueue;
                        leafQueue.initializeUsersMap(username, consumedresource,
                                queueNameAndHopCSLeafQueueUser.get(username).getPendingApplications(),
                                queueNameAndHopCSLeafQueueUser.get(username).getActiveApplications());
    ...
    @Override
    public void dropContainerReservation(RMContainer container, TransactionState transactionState) {
        if (LOG.isDebugEnabled()) {
...
//      app.addPreemptContainer(cont.getContainerId());
    ...
    @Override
    public void killContainer(RMContainer cont, TransactionState transactionState) {
        if (LOG.isDebugEnabled()) {
