...
public class LeafQueue implements CSQueue {
    ...
    private CSQueue parent;    //recovered
    private float capacity;   //recovered
    private float absoluteCapacity;   //from config
    private float maximumCapacity;  // from config
    private float absoluteMaxCapacity;  //from config
    private float absoluteUsedCapacity = 0.0f;   //recovered
    private int userLimit;  //from config
    private float userLimitFactor;  //from config

    private int maxApplications; //from config
    private int maxApplicationsPerUser; //from config

    private float maxAMResourcePerQueuePercent;  //from config
    ...
    private QueueState state;  //from config

    private Map<QueueACL, AccessControlList> acls
            = new HashMap<QueueACL, AccessControlList>();  //from config
    ...
    @Override
    public synchronized void setUsedCapacity(float usedCapacity, TransactionState transactionState) {
        this.usedCapacity = usedCapacity;
        if (transactionState != null) {
            ((TransactionStateImpl) transactionState).getCSQueueInfo().addCSQueue(this.getQueuePath(), this);
        }
    }

    @Override
    public synchronized void setAbsoluteUsedCapacity(float absUsedCapacity, TransactionState transactionState) {
    ...
    public synchronized void initializeUsersMap(String username, Resource consumedResource,
            int pendingApplicatons, int activeApplications) {
        User user = new User();
        user.activeApplications = activeApplications;
        user.pendingApplications = pendingApplicatons;
        user.consumed = consumedResource;
        users.put(username, user);
    }
    ...
    public void initializeQueueInfo(float currentCapacity) {
    ...
    public void submitApplicationAttempt(FiCaSchedulerApp application,
        ...
        synchronized (this) {
            ...
            if (transactionState != null) {
                ((TransactionStateImpl) transactionState).getCSQueueInfo().addCSLeafUsers(userName);
            }
            addApplicationAttempt(application, user, transactionState);
        }

        metrics.submitAppAttempt(userName);
        getParent().submitApplicationAttempt(application, userName, null);
    }
    ...
    private synchronized void activateApplications(TransactionState transactionState) {
    ...
    private synchronized void addApplicationAttempt(FiCaSchedulerApp application, User user, TransactionState transactionState) {
        // Accept 
    ...
    @Override
    public void finishApplicationAttempt(FiCaSchedulerApp application, String queue, TransactionState transactionState) {
        // Careful! Locking order is important!
        synchronized (this) {
    ...
    public synchronized void removeApplicationAttempt(FiCaSchedulerApp application, User user, TransactionState transactionState) {
        ...
        if (user.getTotalApplications() == 0) {
            ...
            if (transactionState != null) {
                ((TransactionStateImpl) transactionState).getCSQueueInfo().addCSLeafQueueUsersToRemove(application.getUser());         
            }
        }
    ...
    public synchronized CSAssignment
            assignContainers(Resource clusterResource, FiCaSchedulerNode node, TransactionState transactionState) {
        ...
        for (FiCaSchedulerApp application : activeApplications) {
          ...
          if (assignment.getSkipped()) {
            ...
            application.subtractSchedulingOpportunity(priority, transactionState);
            continue;
          }
          ...
          if (Resources
            ...
            allocateResource(clusterResource, application, assigned, transactionState);

            // Don't reset scheduling opportunities for non-local assignments
            // otherwise the app will be delayed for each non-local assignment.
            // This helps apps with many off-cluster requests schedule faster.
            if (assignment.getType() != NodeType.OFF_SWITCH) {
              application.resetSchedulingOpportunities(priority, transactionState);
            }
    ...
    private CSAssignment assignContainersOnNode(Resource clusterResource,
            ...
            Priority priority, RMContainer reservedContainer, TransactionState transactionState) {
    ...
    private Resource assignNodeLocalContainers(
        ...
        if (canAssign(application, priority, node, NodeType.NODE_LOCAL,
            ...
            return assignContainer(clusterResource, node, application, priority,
                    nodeLocalResourceRequest, NodeType.NODE_LOCAL, reservedContainer, transactionState);
        }
    ...
    private Resource assignRackLocalContainers(
        ...
        if (canAssign(application, priority, node, NodeType.RACK_LOCAL,
                reservedContainer)) {
            return assignContainer(clusterResource, node, application, priority,
                    rackLocalResourceRequest, NodeType.RACK_LOCAL, reservedContainer, transactionState);
        }
    ...
    private Resource assignOffSwitchContainers(
        ...
        if (canAssign(application, priority, node, NodeType.OFF_SWITCH,
                reservedContainer)) {
            return assignContainer(clusterResource, node, application, priority,
                    offSwitchResourceRequest, NodeType.OFF_SWITCH, reservedContainer, transactionState);
        }
    ...
    private Container getContainer(RMContainer rmContainer,
            ...
            Resource capability, Priority priority, TransactionState ts) {
    ...
    Container createContainer(FiCaSchedulerApp application, FiCaSchedulerNode node,
        ...
        ContainerId containerId = BuilderUtils.newContainerId(application
                .getApplicationAttemptId(), application.getNewContainerId(ts));
    ...
    private Resource assignContainer(Resource clusterResource, FiCaSchedulerNode node,
            ...
            ResourceRequest request, NodeType type, RMContainer rmContainer, TransactionState transactionState) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("assignContainers: node=" + node.getNodeName()
        ...
        Container container
                = getContainer(rmContainer, application, node, capability, priority, transactionState);
      ...
      // Allocate...
            ...
            RMContainer allocatedContainer
                    = application.allocate(type, node, priority, request, container, transactionState);
            ...
            node.allocateContainer(application.getApplicationId(),
                    allocatedContainer, transactionState);
            ...
            reserve(application, priority, node, rmContainer, container, transactionState);
    ...
    private void reserve(FiCaSchedulerApp application, Priority priority,
            FiCaSchedulerNode node, RMContainer rmContainer, Container container, TransactionState transactionState) {
        ...
        // Inform the application 
        rmContainer = application.reserve(node, priority, rmContainer, container, transactionState);
    ...
    private boolean unreserve(FiCaSchedulerApp application, Priority priority,
            FiCaSchedulerNode node, RMContainer rmContainer, TransactionState transactionState) {
    ...
    synchronized void allocateResource(Resource clusterResource,
            FiCaSchedulerApp application, Resource resource, TransactionState transactionState) {
        // Update queue metrics
        Resources.addTo(usedResources, resource);
        CSQueueUtils.updateQueueStatistics(
                resourceCalculator, this, getParent(), clusterResource, minimumAllocation, transactionState);
        ++numContainers;
    ...
    synchronized void releaseResource(Resource clusterResource,
        ...
        CSQueueUtils.updateQueueStatistics(
                resourceCalculator, this, getParent(), clusterResource,
                minimumAllocation, transactionState);
        --numContainers;
    ...
    @Override
    public synchronized void updateClusterResource(Resource clusterResource, TransactionState transactionState) {
    ...
    static class User {
        ...
        public synchronized void submitApplication(String userName, TransactionState transactionState) {
            ++pendingApplications;
            if (transactionState != null) {
                ((TransactionStateImpl) transactionState).getCSQueueInfo().
                        getCSLeafUserInfo(userName).setPendingApplications(pendingApplications);
            }
        }

        public synchronized void activateApplication(String userName, TransactionState transactionState) {
            --pendingApplications;
            ++activeApplications;
            if (transactionState != null) {
                ((TransactionStateImpl) transactionState).getCSQueueInfo().
                        getCSLeafUserInfo(userName).setActiveApplications(activeApplications);
                ((TransactionStateImpl) transactionState).getCSQueueInfo().
                        getCSLeafUserInfo(userName).setPendingApplications(pendingApplications);
        ...
        public synchronized void finishApplication(String userName, boolean wasActive, TransactionState transactionState) {
            if (wasActive) {
                --activeApplications;
                if (transactionState != null) {
                    ((TransactionStateImpl) transactionState).getCSQueueInfo().
        ...
        public synchronized void assignContainer(String userName, Resource resource, TransactionState transactionState) {
            Resources.addTo(consumed, resource);
            if (transactionState != null) {
                ((TransactionStateImpl) transactionState).getCSQueueInfo().
                        getCSLeafUserInfo(userName).setConsumedMemory(consumed.getMemory());
                ((TransactionStateImpl) transactionState).getCSQueueInfo().
                        getCSLeafUserInfo(userName).setConsumedVCores(consumed.getVirtualCores());
            }

        }

        public synchronized void releaseContainer(String userName, Resource resource, TransactionState transactionState) {
    ...
    public void recoverContainer(Resource clusterResource,
        ...
        // Careful! Locking order is important! 
        synchronized (this) {
            allocateResource(clusterResource, application, container.getResource(), null);
        }
