...
public class TestPriceFixingService {
  ...
  public void TestPriceEstimation() throws Exception {
    ...
    PriceFixerService ps = new PriceFixerService(rm.getRMContext()); // RMcontext has to be passed
    ps.init(conf);
    ps.start();
  ...
  public void TestContainerLogServiceUpdatesPrice() throws Exception {
    ...
    ContainersLogsService logService = new ContainersLogsService(rm.
            getRMContext());
  ...
  @Test
  public void TestStreamingServiceUpdatePriceInContainerLogService() throws
          Exception {
    ...
    Assert.assertFalse(rmRT.getRMContext().getGroupMembershipService().
            isLeader());
    // Test - this is a LeadingRT
    Assert.assertTrue(rmRT.getRMContext().getGroupMembershipService().
            isLeadingRT());
    ...
    bomb = new LightWeightRequestHandler(YARNOperationType.TEST) {
      ...
      public Object performTask() throws IOException {
        ...
        if (runningPriceDA != null) {
          runningPriceDA.add(new YarnRunningPrice(PriceType.VARIABLE, 12345678,
                  100f));
        } else {
    ...
    try {
      ...
      bomb = new LightWeightRequestHandler(YARNOperationType.TEST) {
        ...
        public Object performTask() throws IOException {
          ...
          _rpDA.add(new YarnRunningPrice(YarnRunningPrice.PriceType.VARIABLE,
                  123456l, f));
          connector.commit();
