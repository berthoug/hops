...
public class TestRecoverRMContainerImpl {
    ...
    // are retriving data from ha_rmcontainer, container_id is actually fetching from ha_container table. so just insert it here.
     public static void setContainerIdAndState(final String containerId, final RMContainer rmContainer) throws IOException {
        LightWeightRequestHandler setApplicationAttemptIdHandler = new LightWeightRequestHandler(YARNOperationType.TEST) {
            ...
            public Object performTask() throws StorageException {
                ...
                connector.writeLock();
                List<io.hops.metadata.yarn.entity.Container> toAddContainers = new ArrayList<io.hops.metadata.yarn.entity.Container>();
                ContainerDataAccess cDA = (ContainerDataAccess) RMStorageFactory.getDataAccess(ContainerDataAccess.class);
                io.hops.metadata.yarn.entity.Container hopContainer = new io.hops.metadata.yarn.entity.Container(rmContainer.getContainerId().toString(),
    ...
    @Test
    public void testPersistReserveResource() throws IOException {
        ...
        verify(writer).containerStarted(any(RMContainer.class), any(TransactionState.class));

        int rpcID = HopYarnAPIUtilities.setYarnVariables(HopYarnAPIUtilities.RPC);
        byte[] submitAppData = new byte[1];

        RMUtilities.persistAppMasterRPC(rpcID, RPC.Type.SubmitApplication, submitAppData);
        ...
        io.hops.metadata.yarn.entity.RMContainer recoverRmCont= containers.get(rmContainer.getContainerId().toString());
        ...
        assertEquals(recoverRmCont.getReservedPriorityID(), reservedPriority.getPriority());
        assertEquals(recoverRmCont.getReservedMemory(), reservedResource.getMemory());
        assertEquals(recoverRmCont.getReservedVCores(), reservedResource.getVirtualCores());       
    }
}
