...
public class TestFSSchedulerApp {
  ...
  public void testDelayScheduling() {
    ...
    assertEquals(NodeType.NODE_LOCAL, schedulerApp
        .getAllowedLocalityLevel(prio, 10, nodeLocalityThreshold,
            rackLocalityThreshold, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));

    // First five scheduling opportunities should remain node local
    for (int i = 0; i < 5; i++) {
      schedulerApp.addSchedulingOpportunity(prio,new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
      assertEquals(NodeType.NODE_LOCAL, schedulerApp
          .getAllowedLocalityLevel(prio, 10, nodeLocalityThreshold,
              rackLocalityThreshold, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
    }
    ...
    schedulerApp.resetAllowedLocalityLevel(prio, NodeType.NODE_LOCAL,new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    schedulerApp.resetSchedulingOpportunities(prio,new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    assertEquals(NodeType.NODE_LOCAL, schedulerApp
        .getAllowedLocalityLevel(prio, 10, nodeLocalityThreshold,
            rackLocalityThreshold, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
  ...
  public void testDelaySchedulingForContinuousScheduling()
    ...
    assertEquals(NodeType.NODE_LOCAL, schedulerApp
            .getAllowedLocalityLevelByTime(prio, nodeLocalityDelayMs,
                rackLocalityDelayMs, clock.getTime(), new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
    ...
    // manually set back to node local
    schedulerApp.resetAllowedLocalityLevel(prio, NodeType.NODE_LOCAL, new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    schedulerApp.resetSchedulingOpportunities(prio, clock.getTime(), new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    assertEquals(NodeType.NODE_LOCAL, schedulerApp
            .getAllowedLocalityLevelByTime(prio, nodeLocalityDelayMs,
                rackLocalityDelayMs, clock.getTime(), new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
   ...
   */ public void testLocalityLevelWithoutDelays() {
    ...
    assertEquals(NodeType.OFF_SWITCH,
        schedulerApp.getAllowedLocalityLevel(prio, 10, -1.0, -1.0, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
  }
}
