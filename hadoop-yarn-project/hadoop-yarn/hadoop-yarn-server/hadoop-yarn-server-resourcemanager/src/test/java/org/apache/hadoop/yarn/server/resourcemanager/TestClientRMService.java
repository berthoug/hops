...
public class TestClientRMService {
  ...
  @Before
  public void setupDbConf() throws IOException {
    try {
  ...
  public void testGetClusterNodes() throws Exception {
    ...
    request =
        GetClusterNodesRequest.newInstance(EnumSet.of(NodeState.UNHEALTHY));
    nodeReports = client.getClusterNodes(request).getNodeReports();
    Assert.assertEquals(1, nodeReports.size());
    Assert
        .assertEquals("Node is expected to be unhealthy!", NodeState.UNHEALTHY,
            nodeReports.get(0).getNodeState());
  ...
  public void testGetApplicationAttemptReport()
    ...
    ApplicationAttemptId attemptId = ApplicationAttemptId
        .newInstance(ApplicationId.newInstance(123456, 1), 1);
    request.setApplicationAttemptId(attemptId);

    try {
      GetApplicationAttemptReportResponse response =
          rmService.getApplicationAttemptReport(request);
      Assert.assertEquals(attemptId,
          response.getApplicationAttemptReport().getApplicationAttemptId());
    } catch (ApplicationNotFoundException ex) {
  ...
  public void testGetApplicationAttempts() throws YarnException, IOException {
    ...
    GetApplicationAttemptsRequest request =
        recordFactory.newRecordInstance(GetApplicationAttemptsRequest.class);
    ApplicationAttemptId attemptId = ApplicationAttemptId
        .newInstance(ApplicationId.newInstance(123456, 1), 1);
    ...
    try {
      GetApplicationAttemptsResponse response =
          rmService.getApplicationAttempts(request);
      Assert.assertEquals(1, response.getApplicationAttemptList().size());
      Assert.assertEquals(attemptId, response.getApplicationAttemptList().get(0)
  ...
  public void testGetContainerReport() throws YarnException, IOException {
    ...
    GetContainerReportRequest request =
        recordFactory.newRecordInstance(GetContainerReportRequest.class);
    ApplicationAttemptId attemptId = ApplicationAttemptId
        .newInstance(ApplicationId.newInstance(123456, 1), 1);
    ...
    try {
      GetContainerReportResponse response =
          rmService.getContainerReport(request);
      Assert.assertEquals(containerId,
          response.getContainerReport().getContainerId());
    } catch (ApplicationNotFoundException ex) {
      Assert.fail(ex.getMessage());
    }
  }
  ...
  public void testGetContainers() throws YarnException, IOException {
    ...
    GetContainersRequest request =
        recordFactory.newRecordInstance(GetContainersRequest.class);
    ApplicationAttemptId attemptId = ApplicationAttemptId
        .newInstance(ApplicationId.newInstance(123456, 1), 1);
    ContainerId containerId = ContainerId.newInstance(attemptId, 1);
    request.setApplicationAttemptId(attemptId);
    try {
      ...
      Assert.assertEquals(containerId,
          response.getContainerList().get(0).getContainerId());
    } catch (ApplicationNotFoundException ex) {
      Assert.fail(ex.getMessage());
    }
  }
  ...
  public ClientRMService createRMService() throws IOException {
    ...
    ConcurrentHashMap<ApplicationId, RMApp> apps =
        getRMApps(rmContext, yarnScheduler);
  ...
  @Test
  public void testForceKillNonExistingApplication()
      throws YarnException, IOException {
    RMContext rmContext = mock(RMContext.class);
    when(rmContext.getRMApps())
        .thenReturn(new ConcurrentHashMap<ApplicationId, RMApp>());
    ...
    try {
      ...
      Assert.assertEquals(ex.getMessage(), "Trying to kill an absent " +
  ...
  public void testMoveAbsentApplication() throws YarnException {
    ...
    MoveApplicationAcrossQueuesRequest request =
        MoveApplicationAcrossQueuesRequest
            .newInstance(applicationId, "newqueue");
  ...
  public void testGetQueueInfo() throws Exception {
    ...
    mockRMContext(yarnScheduler, rmContext);
    ClientRMService rmService =
        new ClientRMService(rmContext, yarnScheduler, null, null, null, null);
    ...
    List<ApplicationReport> applications =
        queueInfo.getQueueInfo().getApplications();
      ...
      owner.doAs(new PrivilegedExceptionAction<Void>() {
        ...
        public Void run() throws Exception {
          try {
            ...
            Assert.assertTrue(
                ex.getMessage().contains("Client " + owner.getUserName() +
  ...
  public void testAppSubmit() throws Exception {
    ...
    SubmitApplicationRequest submitRequest2 =
        mockSubmitAppRequest(appId2, name, queue);
    submitRequest2.getApplicationSubmissionContext()
        .setApplicationType("matchType");
    try {
    ...
    Assert.assertEquals(appId2,
        getAllApplicationsResponse.getApplicationList().get(0)
            .getApplicationId());
  }
  ...
  public void testGetApplications() throws IOException, YarnException {
    ...
    request = GetApplicationsRequest
        .newInstance(ApplicationsRequestScope.ALL, null, null, null, null, null,
            null, null, null);
    ...
    // Check scope
    request =
        GetApplicationsRequest.newInstance(ApplicationsRequestScope.VIEWABLE);
    ...
    Thread t = new Thread() {
      ...
      public void run() {
        try {
          ...
          Logger.getLogger(TestClientRMService.class.getName())
  ...
  private void mockRMContext(YarnScheduler yarnScheduler, RMContext rmContext)
    ...
    when(
        yarnScheduler.getQueueInfo(eq("testqueue"), anyBoolean(), anyBoolean()))
        .thenReturn(queInfo);
    when(yarnScheduler
        .getQueueInfo(eq("nonexistentqueue"), anyBoolean(), anyBoolean()))
    ...
    ConcurrentHashMap<ApplicationId, RMApp> apps =
        getRMApps(rmContext, yarnScheduler);
    when(rmContext.getRMApps()).thenReturn(apps);
    when(yarnScheduler.getAppsInQueue(eq("testqueue")))
  ...
  private ConcurrentHashMap<ApplicationId, RMApp> getRMApps(RMContext rmContext,
    ...
    apps.put(applicationId1,
        getRMApp(rmContext, yarnScheduler, applicationId1, config,
            "testqueue"));
    apps.put(applicationId2,
        getRMApp(rmContext, yarnScheduler, applicationId2, config, "a"));
    apps.put(applicationId3,
        getRMApp(rmContext, yarnScheduler, applicationId3, config,
            "testqueue"));
    return apps;
  }
  ...
  private List<ApplicationAttemptId> getSchedulerApps(
    ...
    List<ApplicationAttemptId> schedApps =
  ...
  private RMAppImpl getRMApp(RMContext rmContext, YarnScheduler yarnScheduler,
      ApplicationId applicationId3, YarnConfiguration config,
      String queueName) {
    ...
    RMAppAttemptImpl rmAppAttemptImpl =
        spy(new RMAppAttemptImpl(attemptId, rmContext, yarnScheduler, null,
            asContext, config, false));
    Container container = Container
        .newInstance(ContainerId.newInstance(attemptId, 1), null, "", null,
            null, null);
    RMContainerImpl containerimpl =
        spy(new RMContainerImpl(container, attemptId, null, "", rmContext,
            null));
    ...
    when(rmContext.getScheduler()
            .getSchedulerAppInfo(any(ApplicationAttemptId.class)))
        .thenReturn(sAppReport);
    List<RMContainer> rmContainers = new ArrayList<RMContainer>();
    rmContainers.add(containerimpl);
    when(rmContext.getScheduler().getSchedulerAppInfo(attemptId)
  ...
  private static YarnScheduler mockYarnScheduler() {
    YarnScheduler yarnScheduler = mock(CapacityScheduler.class);
    when(yarnScheduler.getMinimumResourceCapability()).thenReturn(Resources
            .createResource(
                YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_MB));
    when(yarnScheduler.getMaximumResourceCapability()).thenReturn(Resources
            .createResource(
                YarnConfiguration.DEFAULT_RM_SCHEDULER_MAXIMUM_ALLOCATION_MB));
    when(yarnScheduler.getAppsInQueue(QUEUE_1)).thenReturn(Arrays
        .asList(getApplicationAttemptId(101), getApplicationAttemptId(102)));
    when(yarnScheduler.getAppsInQueue(QUEUE_2))
        .thenReturn(Arrays.asList(getApplicationAttemptId(103)));
