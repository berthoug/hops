...
public class TestLeafQueue {
  ...
  public void setUp() throws Exception {
    ...
    when(csContext.getContainerTokenSecretManager())
  ...
  private void setupQueueConfiguration(CapacitySchedulerConfiguration conf,
    ...
    final String Q_newRoot =
        CapacitySchedulerConfiguration.ROOT + "." + newRoot;
  ...
  static LeafQueue stubLeafQueue(LeafQueue queue) {
          ...
          public Container answer(InvocationOnMock invocation)
            ...
            final ContainerId containerId = TestUtils
                .getMockContainerId(application,
                    (TransactionState) invocation.getArguments()[4]);

            Container container = TestUtils.getMockContainer(containerId,
                ((FiCaSchedulerNode) (invocation.getArguments()[1]))
                    .getNodeID(), (Resource) (invocation.getArguments()[2]),
    ...
    doNothing().when(parent)
        .completedContainer(any(Resource.class), any(FiCaSchedulerApp.class),
            any(FiCaSchedulerNode.class), any(RMContainer.class),
            any(ContainerStatus.class), any(RMContainerEventType.class),
            any(CSQueue.class), any(TransactionState.class));
  ...
  public void testAppAttemptMetrics() throws Exception {
    ...
    AppAttemptAddedSchedulerEvent addAttemptEvent =
        new AppAttemptAddedSchedulerEvent(appAttemptId_0, false, null);
    cs.handle(addAttemptEvent);

    AppAttemptRemovedSchedulerEvent event =
        new AppAttemptRemovedSchedulerEvent(appAttemptId_0,
            RMAppAttemptState.FAILED, false, null);
    cs.handle(event);
    ...
    event = new AppAttemptRemovedSchedulerEvent(appAttemptId_0,
        RMAppAttemptState.FINISHED, false, null);
    cs.handle(event);
    AppRemovedSchedulerEvent rEvent =
        new AppRemovedSchedulerEvent(appAttemptId_0.getApplicationId(),
            RMAppState.FINISHED, null);
    cs.handle(rEvent);
  ...
  public void testSingleQueueWithOneUser() throws Exception {
    ...
    assertEquals(
        (int) (a.getCapacity() * node_0.getTotalResource().getMemory()),
  ...
  public void testUserLimits() throws Exception {
    ...
    FiCaSchedulerNode node_1 =
        TestUtils.getMockNode(host_1, DEFAULT_RACK, 0, 8 * GB);
    ...
    assertEquals("There should only be 1 active user!", 1,
        a.getActiveUsersManager().getNumActiveUsers());
  ...
  public void testReservation() throws Exception {
    ...
    String host_0 = "127.0.0.1";
    FiCaSchedulerNode node_0 =
        TestUtils.getMockNode(host_0, DEFAULT_RACK, 0, 4 * GB);
  ...
  public void testStolenReservedContainer() throws Exception {
    ...
    String host_1 = "127.0.0.2";
    FiCaSchedulerNode node_1 =
        TestUtils.getMockNode(host_1, DEFAULT_RACK, 0, 4 * GB);
    ...
    appRequests_1.add(TestUtils
        .createResourceRequest(host_0, 4 * GB, 1, true, priority,
            recordFactory));
    appRequests_1.add(TestUtils
        .createResourceRequest(DEFAULT_RACK, 4 * GB, 1, true, priority,
            recordFactory));
    appRequests_1.add(TestUtils
  ...
  public void testReservationExchange() throws Exception {
    ...
    when(csContext.getNumClusterNodes()).thenReturn(numNodes);
    when(csContext.getMaximumResourceCapability())
        .thenReturn(Resources.createResource(4 * GB, 16));
    when(a.getMaximumAllocation())
        .thenReturn(Resources.createResource(4 * GB, 16));
    ...
    assertEquals(4 * GB,
        assignment.getExcessReservation().getContainer().getResource()
            .getMemory());
  }
  ...
  public void testLocalityScheduling() throws Exception {
    ...
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_1, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
    ...
    verify(app_0)
        .allocate(eq(NodeType.OFF_SWITCH), eq(node_2), any(Priority.class),
            ...
            any(TransactionState.class));
    assertEquals(4,
        app_0.getSchedulingOpportunities(priority)); // should NOT reset
  ...
  public void testApplicationPriorityScheduling() throws Exception {
    ...
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_0, 1 * GB, 1, true, priority_1,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_0, 1 * GB, 1, true, priority_1,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_1, 1 * GB, 1, true, priority_1,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_1, 1 * GB, 1, true, priority_1,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(ResourceRequest.ANY, 1 * GB, 2, true,
    ...
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_2, 2 * GB, 1, true, priority_2,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_2, 2 * GB, 1, true, priority_2,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(ResourceRequest.ANY, 2 * GB, 1, true,
    ...
    verify(app_0, never())
        .allocate(any(NodeType.class), eq(node_2), eq(priority_2),
            any(ResourceRequest.class), any(Container.class),
            any(TransactionState.class));
    ...
    verify(app_0, never())
        .allocate(any(NodeType.class), eq(node_0), eq(priority_2),
            any(ResourceRequest.class), any(Container.class),
            any(TransactionState.class));
    assertEquals(0, app_0.getSchedulingOpportunities(priority_2));
    assertEquals(1, app_0.getTotalRequiredResources(priority_2));
    ...
    assertEquals(0, app_0.getSchedulingOpportunities(priority_1));
    assertEquals(0, app_0.getTotalRequiredResources(priority_1));
    verify(app_0).allocate(eq(NodeType.OFF_SWITCH), eq(node_1), eq(priority_2),
        any(ResourceRequest.class), any(Container.class),
        any(TransactionState.class));
  ...
  public void testSchedulingConstraints() throws Exception {
    ...
    String rack_0 = "rack_0";
    FiCaSchedulerNode node_0_0 =
        TestUtils.getMockNode(host_0_0, rack_0, 0, 8 * GB);
    String host_0_1 = "127.0.0.2";
    FiCaSchedulerNode node_0_1 =
        TestUtils.getMockNode(host_0_1, rack_0, 0, 8 * GB);
    
    
    String host_1_0 = "127.0.0.3";
    String rack_1 = "rack_1";
    FiCaSchedulerNode node_1_0 =
        TestUtils.getMockNode(host_1_0, rack_1, 0, 8 * GB);
    ...
    List<ResourceRequest> app_0_requests_0 = new ArrayList<ResourceRequest>();
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_0_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_0_1, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_1_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_1, 1 * GB, 1, true, priority,
                recordFactory));
  ...
  public void testInheritedQueueAcls() throws IOException {
    ...
    assertTrue(
        hasQueueACL(a.getQueueUserAclInfo(user), QueueACL.SUBMIT_APPLICATIONS));
    assertTrue(
        hasQueueACL(b.getQueueUserAclInfo(user), QueueACL.SUBMIT_APPLICATIONS));
    assertFalse(
        hasQueueACL(c.getQueueUserAclInfo(user), QueueACL.SUBMIT_APPLICATIONS));
  ...
  public void testLocalityConstraints() throws Exception {
    ...
    FiCaSchedulerNode node_1_1 =
        TestUtils.getMockNode(host_1_1, rack_1, 0, 8 * GB);
    
    final int numNodes = 4;
    Resource clusterResource =
        Resources.createResource(numNodes * (8 * GB), numNodes * 1);
    when(csContext.getNumClusterNodes()).thenReturn(numNodes);
    ...
    app_0_requests_0.add(TestUtils
            .createResourceRequest(host_1_0, 1 * GB, 1, true, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(rack_1, 1 * GB, 1, false, priority,
                recordFactory));
    app_0_requests_0.add(TestUtils
            .createResourceRequest(ResourceRequest.ANY, 1 * GB, 1, // only one
  ...
  public void testAllocateContainerOnNodeWithoutOffSwitchSpecified()
    ...
    // Setup resource-requests
    Priority priority = TestUtils.createMockPriority(1);
    app_0.updateResourceRequests(Arrays.asList(TestUtils
        .createResourceRequest("127.0.0.1", 1 * GB, 3, true, priority,
  ...
  private CapacitySchedulerContext mockCSContext(
    ...
    when(csContext.getMinimumResourceCapability())
        .thenReturn(Resources.createResource(GB, 1));
    when(csContext.getMaximumResourceCapability())
        .thenReturn(Resources.createResource(2 * GB, 2));
    return csContext;
  }
