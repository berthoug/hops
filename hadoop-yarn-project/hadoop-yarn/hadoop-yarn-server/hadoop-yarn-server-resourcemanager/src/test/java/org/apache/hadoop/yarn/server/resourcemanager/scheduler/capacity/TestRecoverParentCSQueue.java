...
public class TestRecoverParentCSQueue {
    ...
    private static final Log LOG = LogFactory.getLog(TestRecoverParentCSQueue.class);
            ...
            public CSAssignment answer(InvocationOnMock invocation) throws Throwable {
                ...
                if (allocation > 0) {
                    doReturn(new CSAssignment(Resources.none(), type)).
                            when(queue).assignContainers(eq(clusterResource), eq(node), any(TransactionState.class));
    ...
    @Test
    public void testPersistParentCSQueue() throws IOException, InterruptedException {
        ...
        Thread.sleep(500);
        io.hops.metadata.yarn.entity.capacity.CSQueue recoverCSQueue = RMUtilities.getCSQueue(root.getQueuePath());

        assertEquals(recoverCSQueue.getName(), root.getQueueName());
        assertEquals(recoverCSQueue.getPath(), root.getQueuePath());
        assertEquals(recoverCSQueue.getUsedResourceMemory(), root.getUsedResources().getMemory());
        assertEquals(recoverCSQueue.getUsedResourceVCores(), root.getUsedResources().getVirtualCores());
        assertEquals(recoverCSQueue.getUsedCapacity(), root.getUsedCapacity(), DELTA);
        assertEquals(recoverCSQueue.getAbsoluteUsedCapacity(), root.getAbsoluteUsedCapacity(), DELTA);
