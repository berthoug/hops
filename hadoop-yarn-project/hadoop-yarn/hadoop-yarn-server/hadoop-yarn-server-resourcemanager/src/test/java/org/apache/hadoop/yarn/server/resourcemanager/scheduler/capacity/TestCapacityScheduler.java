...
public class TestCapacityScheduler {
  ...
  private void nodeUpdate(
    ...
    NodeUpdateSchedulerEvent nodeUpdate =
        new NodeUpdateSchedulerEvent(node, new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
  ...
  public void testReconnectedNode() throws Exception {
    ...
    cs.handle(new NodeAddedSchedulerEvent(n1, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
    cs.handle(new NodeAddedSchedulerEvent(n2, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));

    Assert.assertEquals(6 * GB, cs.getClusterResources().getMemory());

    // reconnect n1 with downgraded memory
    n1 = MockNodes.newNodeInfo(0, MockNodes.newResource(2 * GB), 1);
    cs.handle(new NodeRemovedSchedulerEvent(n1, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
    cs.handle(new NodeAddedSchedulerEvent(n1, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
  ...
  public void testBlackListNodes() throws Exception {
    ...
    cs.handle(new NodeAddedSchedulerEvent(node, new TransactionStateImpl(-1, TransactionState.TransactionType.RM)));
    ...
    cs.allocate(appAttemptId, Collections.<ResourceRequest>emptyList(),
        ...
        Collections.singletonList(host), new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    Assert.assertFalse(
  ...
  public void testAsyncScheduling() throws Exception {
    ...
    for (int i = 0; i < NODES; ++i) {
      CapacityScheduler.schedule(cs, new TransactionStateImpl(-1, TransactionState.TransactionType.RM));
    }
  }
