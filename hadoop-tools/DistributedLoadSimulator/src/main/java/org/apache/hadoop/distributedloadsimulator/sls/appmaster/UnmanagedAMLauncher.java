 ...
 * env variable Environment.CONTAINER_ID. The AM can be in any language. The AM
 * can register with the RM using the attempt id obtained from the container id
 * and proceed as normal. The client redirects app stdout and stderr to its own
 * stdout and stderr and waits for the AM process to exit. Then it waits for the
 * RM to report app completion.
 */
