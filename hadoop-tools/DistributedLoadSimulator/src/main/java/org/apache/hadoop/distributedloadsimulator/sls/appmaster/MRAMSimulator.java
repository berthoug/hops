...
public class MRAMSimulator extends AMSimulator {
  ...
  public void init(int id, int heartbeatInterval,
    ...
    super.init(id, heartbeatInterval, containerList, rm, se,
            ...
            isTracked, oldAppId, applicatonMasterProtocol, applicationId);
    amtype = "mapreduce";
          ...
          if (cs.getExitStatus() == ContainerExitStatus.SUCCESS) {
            if (assignedMaps.containsKey(containerId)) {
             ...
             // LOG.info(MessageFormat.format("Application {0} has one "
                    //  + "mapper finished ({1}). toatal map {2} - totoareduced {3} - finishedmap -{4}", appId, containerId,mapTotal,reduceFinished,mapFinished));
              finishedContainers++;
...
//      for (ContainerStatus cs : response.getCompletedContainersStatuses())
//      {
//       LOG.info(MessageFormat.format("completedcontatinerid {0} - containerstatus - {1}:", cs.getContainerId(),cs.getExitStatus())); 
//      }
  ...
  protected void checkStop() {
    if (isFinished) {
      //LOG.info(MessageFormat.format("Application {0} setting time to finish.", appId));
