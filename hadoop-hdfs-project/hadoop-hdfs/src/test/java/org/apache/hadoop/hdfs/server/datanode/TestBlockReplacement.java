...
public class TestBlockReplacement {
  private static final Log LOG =
      LogFactory.getLog("org.apache.hadoop.hdfs.TestBlockReplacement");
  ...
  public void testBlockReplacement() throws Exception {
    ...
    try {
      ...
      DFSTestUtil
          .createFile(fs, fileName, DEFAULT_BLOCK_SIZE, REPLICATION_FACTOR,
              r.nextLong());
      ...
      LOG.info(
          "Testcase 1: Proxy " + newNode + " does not contain the block " + b);
      assertFalse(replaceBlock(b, source, newNode, proxies.get(0)));
      // case 2: destination already contains the block
      LOG.info(
          "Testcase 2: Destination " + proxies.get(1) + " contains the block " +
              b);
      ...
      checkBlocks(new DatanodeInfo[]{newNode, proxies.get(0), proxies.get(1)},
          fileName.toString(), DEFAULT_BLOCK_SIZE, REPLICATION_FACTOR, client);
  ...
  private void checkBlocks(DatanodeInfo[] includeNodes, String fileName,
    ...
    LOG.info(
        "Achieved expected replication values in " + (Time.now() - starttime) +
            " msec.");
  }
