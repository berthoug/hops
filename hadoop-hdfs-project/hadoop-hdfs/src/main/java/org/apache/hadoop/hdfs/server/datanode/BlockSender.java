...
class BlockSender implements java.io.Closeable {
  ...
  private static final int TRANSFERTO_BUFFER_SIZE =
      Math.max(HdfsConstants.IO_FILE_BUFFER_SIZE, MIN_BUFFER_WITH_TRANSFERTO);
  ...
  BlockSender(ExtendedBlock block, long startOffset, long length,
    ...
    try {
      ...
      if (replica.getState() == HdfsServerConstants.ReplicaState.RBW) {
      ...
      if (block.getBlockId() >= 0 && (verifyChecksum || sendChecksum)) {
        ...
        if (!corruptChecksumOk || metaIn != null) {
          ...
          checksumIn = new DataInputStream(new BufferedInputStream(metaIn,
      ...
      try {
        NativeIO.POSIX.getCacheManipulator()
            .posixFadviseIfPossible(block.getBlockName(), blockInFd,
                lastCacheDropOffset, offset - lastCacheDropOffset,
  ...
  private static void waitForMinLength(ReplicaInPipeline rbw, long len)
    ...
    if (bytesOnDisk < len) {
      throw new IOException(String
          .format("Need %d bytes, but only %d bytes available", len,
              bytesOnDisk));
    }
  }
  ...
  private int sendPacket(ByteBuffer pkt, int maxChunks, OutputStream out,
    ...
    int numChunks =
        numberOfChunks(dataLen); // Number of chunks be sent in the packet
    ...
    try {
      if (transferTo) {
        ...
        sockOut.transferToFully(fileCh, blockInPosition, dataLen, waitTime,
            transferTime);
  ...
  long sendBlock(DataOutputStream out, OutputStream baseStream,
    ...
    if (isLongRead() && blockInFd != null) {
      ...
      NativeIO.POSIX.getCacheManipulator()
          .posixFadviseIfPossible(block.getBlockName(), blockInFd, 0, 0,
    ...
    try {
      ...
      if (transferTo) {
        ...
        maxChunksPerPacket =
            Math.max(1, numberOfChunks(HdfsConstants.IO_FILE_BUFFER_SIZE));
