 ...
 * block pool a {@link BlockPoolSliceScanner} is created which runs in a
 * separate
...
public class DataBlockScanner implements Runnable {
  ...
  /**
   * Map to find the BlockPoolScanner for a given block pool id. This is
   * updated
   ...
   * verification log file and that is used as the starting block pool id. If
   * no
  ...
  private synchronized String[] getBpIdList() {
    return blockPoolScannerMap.keySet()
        .toArray(new String[blockPoolScannerMap.keySet().size()]);
  }
  ...
  public synchronized void addBlockPool(String blockPoolId) {
    ...
    LOG.info(
        "Added bpid=" + blockPoolId + " to blockPoolScannerMap, new size=" +
  ...
  public static class Servlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      DataNode datanode =
          (DataNode) getServletContext().getAttribute("datanode");
