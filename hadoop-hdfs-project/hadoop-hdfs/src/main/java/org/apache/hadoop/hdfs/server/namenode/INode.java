...
public abstract class INode implements Comparable<byte[]> {
  ...
  private INode(byte[] name, FsPermission permission, INodeDirectory parent,
  ...
  INode(byte[] name, PermissionStatus permissions, INodeDirectory parent,
      long modificationTime, long accessTime) {
    this(name, permissions.getPermission(), parent,
  ...
  INode(INode other) throws IOException {
    ...
    this.userId = UsersGroups.getUserID(userName);
